...
$   create/dir new_gnu:[usr.share.doc.grep]/prot=o:rwed
...
$ if mode .eqs. "install"
$ then
$    copy [.vms]gnv_grep_startup.com -
         new_gnu:[vms_bin]gnv$grep_startup.com
$ else
$    file = "new_gnu:[vms_bin]gnv$grep_startup.com"
...
$!
$!   If gnv$xxx.exe, then:
$!       Source is []gnv$grep.exe
$!       Destination1 is new_gnu:[bin]gnv$grep.exe
...
$!       dest is [usr.share.doc.grep]
$   if f$locate(".doc.grep]", tdir) .lt. tdir_len
$   then
$       source = "sys$disk:[]''tname'''ttype'"
...
$           source = "sys$disk:[]$''tname'''ttype'"
$       endif
$       dest = "new_gnu:[usr.share.doc.grep]''tname'''ttype'"
$       if mode .eqs. "install"
$       then
$           if f$search(dest) .eqs. "" then copy 'source' 'dest'
$       else
